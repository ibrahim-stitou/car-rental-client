...
import { Plus, Trash2, ArrowLeft, FileText, AlertTriangle } from 'lucide-react';
import { useUpdateBillingDocument, useBillingDocument } from '../hooks/use-billing';
...
const schema = z.object({
  type:           z.string().min(1) as z.ZodType<BillingDocumentType>,
...
export function BillingEditView({ id }: Props) {
  const router = useRouter();
  const updateMutation = useUpdateBillingDocument(id);
  const { data: docRes, isLoading } = useBillingDocument(id);
  const { data: agenciesRes } = useAgencies({ per_page: 100 });

  const document = docRes?.data ?? null;
  const agencies = agenciesRes?.data ?? [];

  const form = useForm<FormValues>({
    resolver: zodResolver(schema),
    defaultValues: {
      type: 'FA', agency_id: '', reservation_id: '', client_name: '', client_address: '',
  ...
  // Populate form once the document is loaded
  useEffect(() => {
    if (!document) return;

    const items = (document.items?.length ?? 0) > 0
    ...
    form.reset({
      type:           document.type,
      agency_id:      (document as any).agency?.id ?? '',
      reservation_id: document.reservation_id ?? (document as any).reservation?.id ?? '',
      ...
      delivery_date:  (document as any).delivery_date ?? '',
      items,
    });
  }, [document, form]);
  ...
  const onSubmit = (values: FormValues) => {
    updateMutation.mutate(values as any, {
      onSuccess: () => { toast.success('Document mis à jour'); router.push('/billing'); },
      ...
      <PageContainer scrollable>
        <div className="p-6 max-w-xl mx-auto space-y-6">
          ...
          <Alert variant="destructive">
            ...
            <AlertDescription>
              Ce document a le statut <strong>{document.status}</strong> et ne peut plus être modifié.
              Seuls les documents en statut <strong>Brouillon</strong> sont éditables.
            </AlertDescription>
          </Alert>
        </div>
      </PageContainer>
  ...
  return (
    <PageContainer scrollable>
      <div className="p-6 w-full space-y-6">
        ...
        {document?.document_number?.startsWith('BROUILLON-') && (
          <Alert className="border-amber-200 bg-amber-50 text-amber-800">
            <Info className="h-4 w-4 text-amber-600" />
            <AlertDescription className="text-sm">
              Ce document a une référence temporaire. La référence définitive sera générée lors de la <strong>validation</strong>.
            </AlertDescription>
          </Alert>
        )}
                  ...
                  <Card className="border-2">
                    ...
                    <CardContent className="space-y-3">
                        ...
                        .map(([rate, amount]) => (
                          <div key={rate} className="flex items-center justify-between text-sm">
                            <span className="text-muted-foreground flex items-center gap-1.5">
                              TVA <Badge variant="outline" className="text-[10px] px-1 py-0 h-4">{rate}%</Badge>
                            </span>
                      ...
                      <div className="space-y-2 pt-1">
                        <Button type="button" variant="outline" className="w-full" onClick={() => router.push('/billing')}>
                          Annuler
                        </Button>
