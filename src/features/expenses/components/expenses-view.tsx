...
const CATEGORY_COLORS: Record<string, string> = {
  ...
  vignette: 'bg-green-100 text-green-800',
...
export function ExpensesView() {
  const [tableInstance, setTableInstance] =
    useState<Partial<UseTableReturn<Expense>> | null>(null);
  ...
  /**
   * Categories provenant des paramètres système
   * Même source que ExpenseForm
   */
  const { options: categoryOptions } =
    useParameterOptions('expense_category');
...

    {
      ...
      render: (v) => (
        <span className="font-semibold text-sm">
          {Number(v).toLocaleString('fr-MA')} MAD
        </span>
      ),
    },
      ...
      render: (_v, row) =>
        row.vehicle ? (
          <span className="text-xs font-mono">
            {(row.vehicle as any).registration_number}
          </span>
        ) : (
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      await apiClient.delete(
        apiRoutes.expenses.delete(deleteId)
      );
