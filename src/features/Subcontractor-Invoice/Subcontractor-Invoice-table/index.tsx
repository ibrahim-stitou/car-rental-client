...
export function InvoiceTable<TData, TValue>({
  ...
  // Initial sync from URL params to store state
  useEffect(() => {
    // Only update the store if the URL params are different from store state
    if (page !== currentPage) {
      onPageChange(page);
    }
    if (pageSize !== perPage) {
  ...
  // Update the UI state based on store values
  useEffect(() => {
  ...
  // Keep URL in sync with the store state
  useEffect(() => {
    if (currentPage !== page) {
      setPage(currentPage);
    }
  ...
  }, [currentPage, perPage, setPage, setPageSize]);
  ...
  const { table } = useDataTable({
    ...
    pageCount,
    shallow: false,
    // @ts-ignore - This is necessary due to tanstack table types
    state: {
      pagination: {
  ...
  // Stop showing initial load state once data is loaded
  useEffect(() => {
    if (!loading) {
      setInitialLoad(false);
    }
