      ...
      >
        <div className='text-muted-foreground flex-1 text-sm whitespace-nowrap'>
          {table.selectedRows.length > 0 ? (
            <>
              {table.selectedRows.length} {t('dataTable.page.of')}  {table.data.length} {t('rowsSelected')}
            </>
          ) : (
            <>{table.data.length} {t('rowsTotal')}</>
          )}
        </div>
        <div className='flex flex-col-reverse items-center gap-4 sm:flex-row sm:gap-6 lg:gap-8'>
          ...
          <div className='flex items-center justify-center text-sm font-medium'>
            {t('dataTable.page.page')} {table.currentPage + 1} {t('dataTable.page.of')} {table.pages}
          </div>
          <div className='flex items-center space-x-2'>
            <Button
